   ...
   updateFamilyUsers: async (req, res) => {
        ...
        try {
            ...
            for (const userId of userIds) {
                await connection.query(
                    'INSERT INTO user_family_info (user_id, family_id, privileges, del_flag) VALUES (?, ?, 1, 0) ON DUPLICATE KEY UPDATE privileges = 1, del_flag = 0',
                    [userId, id]
                );
            }
            ...
            for (const scene of scenes) {
                for (const userId of userIds) {
                    await connection.query(
                        'INSERT INTO scene_user_info (scene_id, userid, del_flag) VALUES (?, ?, 0) ON DUPLICATE KEY UPDATE del_flag = 0',
                        [scene.id, userId]
                    );
                }
            }
